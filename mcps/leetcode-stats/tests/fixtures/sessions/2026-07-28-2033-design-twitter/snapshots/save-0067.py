class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        for user in list(self.follows[userId]) + [userId]:
            for idx in range(len(self.tweets[user]) - 1, -1, -1):
                tweet = self.tweets[user][idx]
                heappush(heap, tweet)
                if len(heap) > 10:
                    heappop(heap)
            if len(heap) == 10:
                return [heappop(heap).id for _ in range(10)][::-1]
        return [heappop(heap).id for _ in range(len(heap))][::-1]
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None:
        following = self.follows[followerId]
